...
void onRoadSpeedMsgRecvd( void * dora_context, char *msg)     
{
    ...
    if(speed_task.enable)
    {
        is_swith_speed = true;
        request.reques_type = request.FORWARD_ENABLE_H;

        request.run_speed = speed_task.info;
        Request_h * req_out = &request;
        char * output_data = (char*)req_out;
        std::string out_id = "Request";
        int result = dora_send_output(dora_context, &out_id[0], out_id.length(), output_data, sizeof(Request_h));
    ...
    // std::cout << "speed: " << request.run_speed << std::endl;
...
bool onControlHandleFunction(char *msg)   
{
    ...
    std::cout << "Received control message with type: ";
    switch( req->type )
     {
        case 1:     //Is_stop
        ...
        case 3:   //Is_back
          std::cout << "Is_back" << std::endl;
          backcar_task.enable = req->enable;
          break;

        case 4:   //Is_switch_speed
          std::cout << "Is_switch_speed" << std::endl;
...
void Current_Pose_callback(void *dora_context, char *msg)
{
    ...
    // cout << std::fixed <<" Pub Raw_Path time is: "  << std::setprecision(9) << time 
...
int run(void *dora_context)
{
    ...
    while (true)
    {
        ...
        if (ty == DoraEventType_Input)
        {
            ...
            else if (strncmp("SetSpeed_service", data_id , 16) == 0 || strncmp("SetAEB_service", data_id, 14) == 0 || strncmp("SetStop_service", data_id, 15) == 0)
            {
