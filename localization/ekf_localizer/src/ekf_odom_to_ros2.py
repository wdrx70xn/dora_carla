...
class Operator:
    def __init__(self) -> None:
        ...
        self.odom_data_topic = self.ros2_node.create_topic(
            "/ros2_bridge/ekf/fusion_pose", "nav_msgs::Odometry", self.topic_qos
        )

        self.odom_data_publisher = self.ros2_node.create_publisher(self.odom_data_topic)
    ...
    def on_event(
        ...
        if dora_event["type"] == "INPUT":
            if "DoraGnssPose" == dora_event["id"]:
                ...
                self.odom_data_publisher.publish(pa.array([ros_format_odom]))
