...
static const float STEERING_FILTER_ALPHA = 0.3f; // 低通滤波器系数，值越小越平滑
...
void calculate_angle(void *dora_context){
    ...
    // 获取原始转角
    float raw_steering_angle = -pure_pursuit();
    
    // 应用低通滤波器平滑转角
    float filtered_steering_angle = STEERING_FILTER_ALPHA * raw_steering_angle + 
                                   (1.0f - STEERING_FILTER_ALPHA) * previous_steering_angle;
    
    // 更新前一次转角值
    previous_steering_angle = filtered_steering_angle;
    
    steer_msg.SteeringAngle = filtered_steering_angle;
    // std::cout << "raw_steer: " << raw_steering_angle << " filtered_steer: " << filtered_steering_angle << std::endl;
...
int run(void *dora_context)
{
    ...
    while (true)
    {
        ...
        if (ty == DoraEventType_Input)
        {
            ...
            // std::cout << "steer_msg: " << steer_msg.SteeringAngle << "The count is : " << count <<std::endl;
            ...
            // struct timeval tv_2;
            // gettimeofday(&tv_2, NULL);
            // double time = tv_2.tv_sec + tv_2.tv_usec * 1e-6;
            // cout << std::fixed <<" Pub Angle time is: "  << std::setprecision(9) << time 
            // << "The count is : " << count << std::endl;
