...
void path2rerun(char *msg, size_t data_len, rerun::RecordingStream& rec)
{
    ...
    rec.log(
        ...
        rerun::Transform3D::from_translation_rotation(
            ...
            rerun::RotationAxisAngle{
                {0.0f, 0.0f, 1.0f},
                rerun::Angle::degrees(pose.theta),//车辆坐标系的正前方是y坐标，为了统一到全局，需要逆时针旋转90°
            }
        )
    );
...
void pose2rerun(char *msg, rerun::RecordingStream& rec, const size_t& len)
{
    ...
    pose.theta = j["orientation"]["Heading"].get<double>();
...
int run(void *dora_context, rerun::RecordingStream& rec)
{
    while (true)
    {
        ...
        if (ty == DoraEventType_Input)
        {
            ...
            else if (strncmp("DoraGnssPose", data_id, 8) == 0)
            {
                pose2rerun(data, rec, data_len);
            }
        }    
