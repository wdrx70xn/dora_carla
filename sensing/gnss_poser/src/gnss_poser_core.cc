...
namespace gnss_poser
{
  ...
  GNSSPoser::GNSSPoser(void *dora_context,CoordinateSystem coordinate_system)
  {
    ...
    // aeb
    // nav_sat_fix_origin_.latitude = -0.03274338855385395;
    // nav_sat_fix_origin_.longitude = -0.006386195399866916;
    nav_sat_fix_origin_.latitude = -0.04945589965537067;
    nav_sat_fix_origin_.longitude = -0.0010751675418076599;
    nav_sat_fix_origin_.altitude = 0;
  }
  void GNSSPoser::callbackNavSatFix(const DoraNavSatFix nav_sat_fix_msg_ptr)
  {
      ...
      memcpy(c_json_string, json_string.c_str(), length);
      std::string out_id = "DoraNavSatFix";
      // std::cout<<json_string;
      std::cout << "准备发送消息，输出 ID: " << out_id << std::endl;
      std::cout << "消息内容: " << json_string << std::endl;
      int result = dora_send_output(GNSSPoser_dora_context, &out_id[0], out_id.length(), c_json_string, std::strlen(c_json_string));
      //cout << "The gnss_poser pub time is: "  << tv_1.tv_sec <<","<< tv_1.tv_usec/1000.0f<<" ms " << endl;
      if (result != 0)
      {
...
int run(void *dora_context)
{
    ...
    while(true)
    {
      ...
      if (ty == DoraEventType_Input)
      {
            ...
            if (strncmp("DoraNavSatFix", id, 13) == 0)
            {
              ...
              try 
              {
                ...
                if (j.contains("frame_id")) {
                    ...
                    std::cerr << "JSON 数据中缺少 frame_id 字段" << std::endl;
                    continue;
                }
                // if (j.contains("seq")) {
                //     navSatFix.header.seq = j["seq"];
                // } else {
                //     std::cerr << "JSON 数据中缺少 seq 字段" << std::endl;
                //     continue;
                // }
                // if (j.contains("sec")) {
                //     navSatFix.header.sec = j["sec"];
                // } else {
                //     std::cerr << "JSON 数据中缺少 sec 字段" << std::endl;
                //     continue;
                // }
                // if (j.contains("nanosec")) {
                //     navSatFix.header.nanosec = j["nanosec"];
                // } else {
                //     std::cerr << "JSON 数据中缺少 nanosec 字段" << std::endl;
                //     continue;
                // }
                if (j.contains("latitude") && j["latitude"].is_number()) {
                    navSatFix.latitude = j["latitude"].get<double>();
                } else {
                    std::cerr << "JSON 数据中缺少 latitude 字段或类型不正确" << std::endl;
                    continue;
                }
                if (j.contains("longitude") && j["longitude"].is_number()) {
                    navSatFix.longitude = j["longitude"].get<double>();
                } else {
                    std::cerr << "JSON 数据中缺少 longitude 字段或类型不正确" << std::endl;
                    continue;
                }
                if (j.contains("altitude") && j["altitude"].is_number()) {
                    navSatFix.altitude = j["altitude"].get<double>();
                } else {
                    std::cerr << "JSON 数据中缺少 altitude 字段或类型不正确" << std::endl;
                    continue;
                }
                // navSatFix.position_covariance = j["position_covariance"];
                // navSatFix.position_covariance_type = j["position_covariance_type"];
                ...
                //     navSatFix.status.status = j["status"];
                // } else {
                //     std::cerr << "JSON 数据中缺少 status 字段" << std::endl;
                //     continue;
                // }
                // navSatFix.status.service = j["service"];
                // 将 position_covariance 中的元素逐个添加到 position_covariance 向量中
                // if (j.contains("position_covariance") && j["position_covariance"].is_array()) {
                //     for (auto& elem : j["position_covariance"]) {
                //         navSatFix.position_covariance.push_back(elem);
                //     }
                // } else {
                //     std::cerr << "JSON 数据中缺少 position_covariance 字段或类型不正确" << std::endl;
                // }
                ...
                // cout << "NavSatFix event count: "<<cnt_run<<" data_seq "<< j["seq"]<<" Rec nav time is: "  
...
int main()
{
  ...
  const char* filename_1 = "output.txt";
  remove(filename_1);
